...
train_iters = max_iters * grad_accum_steps  # micro-steps
print_interval = 1  # optimizer steps
eval_interval = 40  # optimizer steps
...
class Block(nn.Module):
    ...
    def __init__(self, hidden_size, ffn_hidden_size, num_attention_heads):
        ...
        # Compile RoPE + QK Norm to fuse surrounding tensor ops
        self._pre_attention = torch.compile(self._pre_attention)

    def _pre_attention(self, q, k, v, rotary_pos_emb):
        """RoPE + QK RMSNorm — compiled to fuse surrounding ops."""
...
T_max = max(1, max_iters - warmup_iters)
...
last_print_step = 0  # Track which optimizer step we last printed at
...
for iter in range(train_iters):
    should_step = ((iter + 1) % grad_accum_steps == 0)
    step = (iter + 1) // grad_accum_steps
    should_print = should_step and (step % print_interval == 0 or step == max_iters)
    ...
    if USE_FSDP2:
        model.set_requires_gradient_sync(should_step)  
    ...
    if should_print and master_process:
        ...
        # Calculate actual optimizer steps since last print
        steps_since_last_print = max(1, step - last_print_step)
        tokens_since_last_print = batch_size * block_size * grad_accum_steps * steps_since_last_print * ddp_world_size
        tok_per_sec = int(tokens_since_last_print / dt)
        avg_step_time = dt / steps_since_last_print
        ...
        # Calculate MFU
        flops_achieved = flops_per_iter * (steps_since_last_print * grad_accum_steps) / dt
        mfu = flops_achieved / total_peak_flops * 100  # as percentage
        
        print(f"step {step}: train loss {loss_value:.4f}")
        print(f"avg step time: {avg_step_time*1000:.2f}ms | tok/sec: {tok_per_sec:,}")
    ...
    if should_step and step % eval_interval == 0:
