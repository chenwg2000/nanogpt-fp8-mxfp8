...
| `pytorch/csrc/extensions/gemm.cpp` | +68 | Add `transpose_mxfp8_operand` lambda before `swizzle_scales_for_gemm`. For MXFP8 non-TN GEMMs on SM 12.0: transposes columnwise data `[M,K] -> [K,M]`, transposes columnwise scales, creates new TensorWrapper with transposed data as rowwise, sets `transa=true, transb=false`. Keeps transposed tensors alive in a vector during GEMM. |
